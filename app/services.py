...
def validate_name(raw_name: str) -> ServiceResult:
    cfg = load_config()
...
async def register_member(member: discord.Member, requested_name: str, actor_id: int | None = None) -> ServiceResult:
    async with _lock_for_user(member.id):
        checked = validate_name(requested_name)
        ...
        try:
            ...
            await member.edit(nick=checked.normalized_name, reason="User registration")
...
async def restore_member(member: discord.Member, actor_id: int | None = None) -> ServiceResult:
    async with _lock_for_user(member.id):
        ...
        try:
            ...
            await member.edit(nick=target_name, reason="Restore from database")
