...
def load_badwords() -> dict:
    ...
    try:
        ...
        normalized_data = {"words": sorted(cleaned_words)}
        save_badwords(normalized_data)
...
def analyze_name(name: str) -> dict:
    ...
    return {
        ...
        "ok": valid and not found_profanity,
        "reason": reason,
...
def make_registration_embed() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• формат имени автоматически приводится к красивому виду"
        ),
        inline=False
    )
...
def build_user_db_embed(member: discord.Member, registration: sqlite3.Row) -> discord.Embed:
    embed = discord.Embed(
...
def build_name_history_embed(member: discord.Member, registration: Optional[sqlite3.Row], rows: list[sqlite3.Row]) -> discord.Embed:
    embed = discord.Embed(
        title="🕘 История имён",
        description=f"История для {member.mention}",
    ...
    if registration:
        embed.add_field(
            ...
            value=f"**{registration['real_name']}**",
            inline=False
        )
        embed.add_field(
            name="Текущий ник",
            value=f"`{registration['nickname']}`",
            inline=False
        )
...
def build_system_check_embed(guild: discord.Guild) -> discord.Embed:
    me = guild.me
    perms = guild.me.guild_permissions if me else None
    ...
    embed.add_field(
        name="Роли",
        value=(
            f"Не зарегистрирован: {unregistered_role.mention if unregistered_role else '❌ не найдена'}\n"
            f"Участник: {member_role.mention if member_role else '❌ не найдена'}"
        ),
        inline=False
    )
...
async def apply_registration(
    ...
    if not unregistered_role:
        return False, "Роль `Не зарегистрирован` не найдена.", None
    if not member_role:
        return False, "Роль `Участник` не найдена.", None
    ...
    try:
        ...
        return False, "Ник изменён, но при обновлении ролей произошла ошибка.", None
...
class BadwordsListView(discord.ui.View):
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(
            text=f"Страница пользовательского списка {self.page + 1}/{self.max_page + 1}"
        )
        return embed
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message(
                embed=make_warning_embed(
                    "Нет доступа",
                    "Эта панель предназначена не для вас."
                ),
                ephemeral=True
            )
            return False
...
class AdminUserActionView(discord.ui.View):
    ...
    async def history_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if member is None:
            await interaction.response.send_message(embed=make_error_embed("Ошибка", "Пользователь не найден."), ephemeral=True)
            return
        ...
        embed = build_name_history_embed(member, registration, rows)
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @discord.ui.button(label="Сбросить", style=discord.ButtonStyle.danger, emoji="♻️")
...
class AdminUserSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if member is None or registration is None:
            await interaction.response.send_message(
                embed=make_error_embed("Ошибка", "Не удалось найти пользователя или его запись в базе."),
                ephemeral=True
            )
            return

        embed = build_user_db_embed(member, registration)
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message(
                embed=make_warning_embed(
                    "Нет доступа",
                    "Эта админ-панель предназначена не для вас."
                ),
                ephemeral=True
            )
            return False
...
async def check_name_slash(interaction: discord.Interaction, name: str):
    ...
    embed.add_field(
        name="Формат",
        value="✅ корректный" if analysis["valid"] else "❌ некорректный",
        inline=True
    )
    embed.add_field(
        name="Антимат",
        value="❌ найдено совпадение" if analysis["found_profanity"] else "✅ не найдено",
        inline=True
    )
    ...
    if analysis["changed_case"]:
        embed.add_field(
            name="Подсказка",
            value=f"Лучше использовать: **{analysis['suggested']}**",
            inline=False
        )
...
async def db_user_slash(interaction: discord.Interaction, member: discord.Member):
    ...
    await interaction.response.send_message(embed=build_user_db_embed(member, registration), view=view, ephemeral=True)
...
async def slash_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        ...
        message_embed = make_error_embed("Ошибка", str(error))
